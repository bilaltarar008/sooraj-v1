...
function StatCard({ icon, label, target, index, isInView }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: "1.9rem", fontWeight: "800", color: "#0c6b2c" }}>
        <motion.span>{rounded}</motion.span>+
      </div>
...
export default function StatsSection() {
  ...
  const stats = [
    { label: t("stats.members"), target: 15 },
    { label: t("stats.projects"), target: 3500 },
    { label: t("stats.clients"), target: 5000 },
    { label: t("stats.awards"), target: 25 },
  ];

  return (
    <section ref={ref} style={{ padding: "38px 0", background: "#f4fff7" }}>
